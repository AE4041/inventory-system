...
const selectVariants = cva(`group inline-flex cursor-pointer relative select-none rounded-md
...
function SelectTrigger({
    ...
    return (
        <PRSelect.Trigger
            className={cn('flex items-center w-full outline-none cursor-pointer', className)}
            {...props}>
            {children}
            <PRSelect.Indicator
                className="flex items-center justify-center shrink-0 bg-transparent text-surface-400 w-10 rounded-e-md">
                <ChevronDown />
            </PRSelect.Indicator>
        </PRSelect.Trigger>
    );
...
function SelectValue({
    ...
    return (
        <PRSelect.Value
            className={cn(`block whitespace-nowrap overflow-hidden flex-auto w-[1%]
...
function SelectClear({
    ...
    return (
        <PRSelect.Clear
            className={cn('text-surface-400 absolute top-1/2 -mt-2.5 inset-e-10', className)}
            {...props} />
    );
}
...
function SelectPopup({
    ...
    return (
        <PRSelect.Popup
            className={cn(`rounded-md min-w-(--px-positioner-anchor-width)
...
function SelectFilter({
    ...
    return (
        <PRSelect.Filter
            className={cn(`w-full appearance-none rounded-md outline-hidden
            ...
            ps-3 pe-10 py-2
            transition-colors duration-200 shadow-[0_1px_2px_0_rgba(18,18,23,0.05)]`, className)}
            {...props} />
    );
}
                    ...
                    instance.options?.map((option, index) => {
                        ...
                        return (
                            <SelectOption key={index} index={index} group={group}>
                                {group ? instance.listbox?.getOptionGroupLabel((option).optionGroup) : instance.listbox?.getOptionLabel(option)}
                            </SelectOption>
                        );
                    }))}
        </PRSelect.List>
...
function SelectOption({
    ...
    return (
        <PRSelect.Option
            className={cn(`whitespace-nowrap relative overflow-hidden flex items-center gap-2
            px-2.5 py-1 border-none text-sm 
            select-none [&_svg]:pointer-events-none [&_svg]:shrink-0`, group
