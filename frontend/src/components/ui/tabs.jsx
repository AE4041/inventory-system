...
function TabsList({
    ...
    return (
        <PRTabs.List
            className={cn(
                'flex relative overflow-hidden bg-surface-0 dark:bg-surface-900 border-b border-surface-200 dark:border-surface-700',
                className
            )}
            {...props}>
            <PRTabs.Prev
        ...
        transition-colors duration-200 inset-s-0`}>
            ...
            <PRTabs.Content
                className="grow relative flex min-h-0 overflow-x-auto overflow-y-clip overscroll-y-contain overscroll-x-auto scroll-smooth scrollbar-none">
                {children}
                <PRTabs.Indicator
                    className="absolute z-10 block w-(--px-active-bar-width) left-(--px-active-bar-left) h-px bottom-0 bg-primary transition-[left,width] duration-[250ms] ease-[cubic-bezier(0.35,0,0.25,1)]" />
            </PRTabs.Content>
            <PRTabs.Next
                className={`absolute shrink-0 top-0 z-20 h-full flex items-center justify-center cursor-pointer
        bg-surface-0 dark:bg-surface-900 text-surface-500 dark:text-surface-400 hover:text-surface-700 dark:hover:text-surface-0 w-9
