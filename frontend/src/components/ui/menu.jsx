...
function MenuList({
    ...
    return (
        <PRMenu.List
            className={cn(`m-0 p-1 list-none flex flex-col gap-1 outline-none
            [&:has([role=menuitemcheckbox])_[data-part=label]]:ps-8
            [&:has([role=menuitemradio])_[data-part=label]]:ps-8`, className)}
            {...props} />
    );
}
...
function MenuSeparator({
    ...
    return (
        <PRMenu.Separator
            className={cn('border-t border-surface-200 dark:border-surface-700', className)}
            {...props} />
    );
}
