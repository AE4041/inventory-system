...
const inputTextVariants = cva(`appearance-none rounded-md w-full outline-hidden file:font-medium
...
function InputText({
    ...
    return (
        <PRInputText
            size={size}
            variant={variant}
            className={cn(inputTextVariants({ size, variant, fluid, className }))}
            {...props} />
    );
}
