...
export default function ReceiptDialog({ sale, visible, onHide }) {
  ...
  return (
    <Dialog header={`Receipt ${sale.receiptNumber}`} visible={visible} onHide={onHide} style={{ width: "28rem" }}>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-100 dark:divide-gray-700 mb-3">
        {sale.items?.map((item) => (
          <div key={item.id} className="flex justify-between px-3 py-2 text-sm">
            ...
            <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(item.total, currency)}</span>
          </div>
        ))}
      </div>

      <div className="text-right text-sm space-y-0.5 mb-4">
        <p className="text-gray-500 dark:text-gray-400">Subtotal: {formatCurrency(sale.subtotal, currency)}</p>
        {Number(sale.discount) > 0 && <p className="text-gray-500 dark:text-gray-400">Discount: -{formatCurrency(sale.discount, currency)}</p>}
        {Number(sale.tax) > 0 && <p className="text-gray-500 dark:text-gray-400">Tax: {formatCurrency(sale.tax, currency)}</p>}
        <p className="text-lg font-semibold text-gray-900 dark:text-white">Total: {formatCurrency(sale.total, currency)}</p>
      </div>
